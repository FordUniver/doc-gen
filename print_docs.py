...
# mathlib3 is deprecated. Where we know the mathlib4 successor of a file
# (via the wiki-maintained port-status YAML), declare it as the canonical
# URL so search engines transfer ranking to the maintained library. Pages
# without a mapping omit canonical entirely — Google explicitly rejects
# blanket many-to-one canonicals to a root URL.
MATHLIB4_DOCS_ROOT = 'https://leanprover-community.github.io/mathlib4_docs/'

def _read_yaml_with_fence(yaml_path):
  """Read yaml, stripping a leading ```...``` markdown fence if present."""
...
def _load_port_map(port_yaml, resolved_yaml=None):
  """Return ({mathlib3_module: mathlib4_docs_url}, {modules to noindex}).

  port_status.yaml maps each mathlib3 module to its mathlib4 path at port
  time. port_status_resolved.yaml (sidecar, generated by
  scripts/resolve_mathlib4_paths.py) updates that path to the current
  mathlib4 HEAD by following git rename history, and tags each entry
  with a resolution_status. Modules whose status is `deleted` or
  `unknown` (no successor in mathlib4) get added to the noindex set so
  the renderer emits `<meta name="robots" content="noindex,follow">`
  rather than canonicalising — Google explicitly rejects blanket
  many-to-one canonicals to a root URL.
  """
  port = _read_yaml_with_fence(port_yaml) or {}
  resolved = _read_yaml_with_fence(resolved_yaml) or {} if resolved_yaml else {}

  canonical = {}
  noindex = set()
  for module, entry in port.items():
    if not isinstance(entry, dict):
      continue
    res = resolved.get(module) if isinstance(resolved.get(module), dict) else None
    status = res.get('resolution_status') if res else None

    # Honour the resolver's verdict when present.
    if status in ('deleted', 'unknown', 'unverified_404'):
      noindex.add(module)
      continue

    current = (res.get('current_mathlib4_file') if res else None) \
              or entry.get('mathlib4_file')
    if not entry.get('ported') or not current:
      # Unported file: don't canonicalise, but mathlib3 page is still
      # the original source — keep indexable. (Renderer can be tuned
...
_port_map, _noindex_modules = _load_port_map(
  os.path.join(root, 'port_status.yaml'),
  os.path.join(root, 'port_status_resolved.yaml'),
)

def _module_of(path):
  if not path.endswith('.html'):
    return None
  return path[:-len('.html')].replace('/', '.')

def get_canonical_url(path, project='mathlib'):
  # Only mathlib (proper) modules have port-status entries; archive,
  # counterexamples, and the docs/extra/test category pages do not.
  if project != 'mathlib':
    return None
  module = _module_of(path)
  return _port_map.get(module) if module else None

def get_noindex(path, project='mathlib'):
  if project != 'mathlib':
    return False
  module = _module_of(path)
  return module in _noindex_modules if module else False
...
def write_html_files(partition, loc_map, notes, mod_docs, instances, instances_for, tactic_docs, bib):
  ...
  with open_outfile('foundational_types.html') as out:
      out.write(env.get_template('foundational_types.j2').render(
        canonical_url = get_canonical_url(current_filename),
        active_path='',
  ...
  for filename, decls in partition.items():
    ...
    with open_outfile(html_root + filename.url) as out:
      ...
      out.write(env.get_template('module.j2').render(
        ...
        noindex = get_noindex(current_filename, project=filename.project),
...
def copy_css_and_js(path, use_symlinks):
  ...
  cp('color_scheme.js', path+'color_scheme.js')
  cp('STIXTwoMath.woff2', path+'STIXTwoMath.woff2')
  cp('STIXlicense.txt', path+'STIXlicense.txt')
